...
- 默认测试模型：`gpt-5.5`
- 默认单账号超时：`10` 秒
- 默认勾选“按页面滚动扫描过滤账号”，适合只检测当前分组/当前页面列表
...
1. 先在 Sub2API 后台筛选好平台、分组、状态或搜索条件。
2. 保持“按页面滚动扫描过滤账号”勾选。
3. 确认测试模型是 `gpt-5.5`。
4. 点击“开始巡检”。
5. 巡检结束后点击“复制失败邮箱”，交给客户或运维处理。

如果你的筛选条件没有体现在地址栏，也可以在“额外查询参数”里手动写：

```text
group=xxx&subscription=plus
```
...
- Page-scroll filtering is enabled by default, useful for testing the current group/current visible account list
...
1. Filter platform, group, status, or search terms in the Sub2API admin UI.
2. Keep “page-scroll filtering” checked.
3. Confirm the test model is `gpt-5.5`.
4. Click “Start”.
5. After the run finishes, click “Copy failed emails”.

If your filters are not reflected in the URL, type extra query parameters manually:
